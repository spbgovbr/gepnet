...
abstract class DBDataWrapper extends DataWrapper
{
    ...
    */
    ...
    public function select($source)
    {
        ...
        return $this->query($this->select_query($select, $source->get_source(), $where, $sort, $source->get_start(),
            $source->get_count()));
    }
        ...
        for ($i = 0; $i < sizeof($rules); $i++) {
            if (is_string($rules[$i])) {
                ...
                if ($filtervalue != "") {
                    if (!$filteroperation) {
                        array_push($sql,
                            $this->escape_name($rules[$i]["name"]) . " LIKE '%" . $this->escape($filtervalue) . "%'");
                    } else {
                        ...
                        array_push($sql,
                            $this->escape_name($rules[$i]["name"]) . " " . $filteroperation . " " . $filtervalue);
                    }
                }
            }
        }
    ...
    */
    protected function update_query($data, $request)
    {
        ...
        $sql .= implode(",",
                $temp) . " WHERE " . $this->escape_name($this->config->id["db_name"]) . "='" . $this->escape($data->get_id()) . "'";
    ...
    */
    protected function insert_query($data, $request)
    {
        ...
        $sql = "INSERT INTO " . $request->get_source() . "(" . implode(",", $temp_n) . ") VALUES (" . implode(",",
                $temp_v) . ")";
