...
$(function () {
    ...
    $form.validate({
        ...
        submitHandler: function (form) {
            ...
            enviar_ajax("/planodeacao/tpa/informacoesiniciais/format/json", "form#form-gerencia", function (data) {
                if (data.success) {
                    $("#submitbutton").prop("disabled", false);
                } else {
                    $.pnotify({
    ...
    $("#transformarbutton").on('click', function (event) {
        event.preventDefault();
        var idplanodeacao = $("#idplanodeacao").val();
        window.location.href = base_url + "/planodeacao/tpa/transformaremprojeto/idplanodeacao/" + idplanodeacao;
    });
