...
$(function () {
    ...
    actions = {
        ...
        inserir: {
            url: base_url + '/projeto/atareuniao/cadastrar/format/json',
            dialog: $('#dialog-inserir')
        },
        editar: {
            url: base_url + '/projeto/atareuniao/editar/format/json',
            dialog: $('#dialog-editar')
        },
        excluir: {
            url: base_url + '/projeto/atareuniao/excluir/format/json',
    ...
    grid = jQuery("#list-grid-ata").jqGrid({
        //caption: "Documentos",
        url: base_url + "/projeto/atareuniao/grid-ata/idprojeto/" + $('#idprojeto').val(),
        datatype: "json",
        mtype: 'post',
        width: '645',
        height: '200',
            ...
            success: function (data) {
                ...
                $form.validate({
                    ...
                    submitHandler: function (form) {
                        enviar_ajax("/projeto/atareuniao/cadastrar/format/json", "form#form-ata", function (data) {
                            if (data.success) {
                                resetFormAta();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $form.validate({
                    ...
                    validClass: 'success',
                    submitHandler: function (form) {
                        enviar_ajax("/projeto/atareuniao/editar/format/json", "form#form-ata", function (data) {
                            if (data.success) {
                                grid.trigger('reloadGrid');
                            }
                        });
...
//
    ...
    actions.excluir.dialog.dialog({
        ...
        buttons: {
            'Excluir': function () {
                ...
                ajax_arrparams("/projeto/atareuniao/excluir/format/json", arrParams, function (data) {
                    if (data.success) {
                        grid.trigger('reloadGrid');
    ...
    function formatadorLink(cellvalue, options, rowObject) {
        var r = rowObject,
            params = '',
            url = {
                editar: base_url + '/projeto/atareuniao/editar',
                excluir: base_url + '/projeto/atareuniao/excluir',
                detalhar: base_url + '/projeto/atareuniao/detalhar',
                imprimir: base_url + '/projeto/atareuniao/imprimir/print/one',
                imprimirbook: base_url + '/projeto/atareuniao/imprimir-word',
            };
        params = '/idata/' + r[4] + '/idprojeto/' + r[6];
...
//        teste = rowObject;
        ...
        return '<a data-target="#dialog-detalhar" class="btn actionfrm detalhar" title="Detalhar" data-id="' + cellvalue + '" href="' + url.detalhar + params + '"><i class="icon-tasks"></i></a>' +
            ...
            '<a data-target="#dialog-print" class="btn actionfrm print" title="Imprimir" target="_blank" data-id="' + cellvalue + '" href="' + url.imprimir + params + '"><i class="icon-print"></i></a>' +
            '<a data-target="#dialog-print" class="btn actionfrm printword" title="Exportar para Word" target="_blank" data-id="' + cellvalue + '" href="' + url.imprimirbook + params + '"><i class="icon-book"></i></a>'
    ...
    $('#btnpesquisar').click(function (e) {
        ...
        if ($("form#form-ata-pesquisar").valid()) {
            grid.setGridParam({
                url: base_url + "/projeto/atareuniao/grid-ata?" + $("form#form-ata-pesquisar").serialize(),
                page: 1
            }).trigger("reloadGrid");
        }
    });
