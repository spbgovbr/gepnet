...
$(function() {
    ...
    $dialogEditar.dialog({
        ...
        open: function(event, ui) {
            $('#form-pessoa input[name=nompessoa], #form-pessoa input[name=numcpf], #form-pessoa input[name=nummatricula], #form-pessoa input[name=token], #form-pessoa select[name=domcargo]')
                    //.attr('readonly', false)
                    //.focus(function() {
                //$(this).blur();
           // });
        ...
        open: function(event, ui) {
           ...
           // });
            ...
            $("form#form-pessoa").validate({
                ...
                submitHandler: function(form) {
                    //console.log('enviar');
                    enviar_ajax("/cadastro/pessoa/add/format/json", "form#form-pessoa", function() {
                        grid.trigger("reloadGrid");
                    });
                }
            });
    ...
    $(document.body).on('click', "#cadastrar", function(event) {
        event.preventDefault();
        var
                $this = $(this);
        ...
        $.ajax({
            ...
            processData: false,
            success: function(data) {
                $dialogAdd.html(data).dialog('open');
            },
            error: function() {
                $.pnotify({
                    text: 'Falha ao enviar a requisição',
