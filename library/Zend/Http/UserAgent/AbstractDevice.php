    ...
    implements Zend_Http_UserAgent_Device
{
    ...
    protected function _getDefaultFeatures()
    {
        ...
        if (isset($this->_server['server_software'])) {
            if (strpos($this->_server['server_software'],
                    'Apache') !== false || strpos($this->_server['server_software'], 'LiteSpeed') !== false) {
                $server['version'] = 1;
    ...
    public static function extractFromUserAgent($userAgent)
    {
        ...
        if (isset($result['others'])) {
            preg_match_all('/(([^\/\s]*)(\/)?([^\/\(\)\s]*)?)(\s\((([^\)]*)*)\))?/i', $result['others']['full'],
                $match2);
        }
        ...
        if (isset($result['compatibility_flag']) && isset($result['comment'])) {
            $compatibleOrIe = ($result['compatibility_flag'] == 'compatible' || strpos($result['comment']['full'],
                    "MSIE") !== false);
        }
        ...
        if (isset($result['others'])) {
            ...
            if ($result['others']['detail'][0][1] == 'Gecko') {
                $searchRV = true;
                if (!empty($result['others']['detail'][1][1]) && !empty($result['others']['detail'][count($result['others']['detail']) - 1][2]) || strpos(strtolower($result['others']['full']),
                        'opera') !== false) {
                    $searchRV = false;
