    ...
    public function loadXml($configurationXml)
    {
        ...
        if ($configurationXml->DataSources->PerformanceCounters->Subscriptions
            ...
            foreach ($subscriptions as $subscription) {
                $this->DataSources->PerformanceCounters->addSubscription((string)$subscription->CounterSpecifier,
        ...
        if ($configurationXml->DataSources->Directories->Subscriptions
            ...
            foreach ($subscriptions as $subscription) {
                $this->DataSources->Directories->addSubscription((string)$subscription->Path,
                    (string)$subscription->Container, (int)$subscription->DirectoryQuotaInMB);
            }
        }
    }
