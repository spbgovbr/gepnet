...
class Zend_Search_Lucene_Search_Query_Fuzzy extends Zend_Search_Lucene_Search_Query
{
    ...
    {
        $this->_maxDistances[$length] = (int)((1 - $this->_minimumSimilarity) * (min($termLength,
                    $length) + $prefixLength));
                    ...
                    if ($termRestLength == 0) {
                        ...
                        if (strlen($target) == 0) {
                            ...
                            if ($maxDistance < abs($termRestLength - strlen($target))) {
                                ...
                                $similarity = 1 - levenshtein($termRest,
                                        $target) / ($prefixUtf8Length + min($termRestLength, strlen($target)));
                            }
                        }
                    }
