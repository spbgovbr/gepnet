...
class Zend_Pdf_Outline_Loaded extends Zend_Pdf_Outline
{
        ...
        if ($updateChildNavigation) {
            ...
            foreach ($this->childOutlines as $childOutline) {
                ...
                if ($lastChild === null) {
                    ...
                    $childOutlineDictionary = $childOutline->dumpOutline($factory, $updateChildNavigation,
                        $this->_outlineDictionary, $lastChild, $processedOutlines);
    ...
    {
        printf(":%3d:%s:%s:%s%s  :\n", count($this->childOutlines), $this->isItalic() ? 'i' : ' ',
            $this->isBold() ? 'b' : ' ', str_pad('', 4 * $level), $this->getTitle());
