...
abstract class Zend_Date_DateObject
{
    ...
    protected function setUnixTimestamp($timestamp = null)
    {
        ...
        if (is_numeric($timestamp)) {
            ...
            if ($timestamp === null) {
                ...
                throw new Zend_Date_Exception('\'' . $timestamp . '\' is not a valid UNIX timestamp', 0, null,
                    $timestamp);
            }
        }
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('Zend_DateObject_mkTime_' . $this->_offset . '_' . $year . $month . $day . '_' . $hour . $minute . $second . '_' . (int)$gmt,
                '-', '_');
