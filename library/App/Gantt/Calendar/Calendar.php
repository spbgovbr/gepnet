<?php

class App_Gantt_Calendar_Calendar
{

    static $now = 0;

    function __construct()
    {
        App_Gantt_Calendar_Calendar::$now = time();
    }

    function years($start, $end)
    {
        $array = array();
        foreach (range($start, $end) as $year) {
            $array[] = $this->year($year);
        }
        return new App_Gantt_Calendar_CalendarIterator($array);
    }

    function year($year)
    {
        return new App_Gantt_Calendar_CalendarYear($year, 1, 1, 0, 0, 0);
    }

    function months($year = false)
    {
        $year = new App_Gantt_Calendar_CalendarYear($year, 1, 1, 0, 0, 0);
        return $year->months();
    }

    function month($year, $month)
    {
        return new App_Gantt_Calendar_CalendarMonth($year, $month, 1, 0, 0);
    }

    function week($year = false, $week = false)
    {
        return new App_Gantt_Calendar_CalendarWeek($year, $week);
    }

    function days($year = false)
    {
        $year = new App_Gantt_Calendar_CalendarYear($year);
        return $year->days();
    }

    function day($year = false, $month = false, $day = false)
    {
        return new App_Gantt_Calendar_CalendarDay($year, $month, $day);
    }

    function date()
    {

        $args = func_get_args();

        if (count($args) > 1) {

            $year = isset($args[0]) ? $args[0] : false;
            $month = isset($args[1]) ? $args[1] : 1;
            $day = isset($args[2]) ? $args[2] : 1;
            $hour = isset($args[3]) ? $args[3] : 0;
            $minute = isset($args[4]) ? $args[4] : 0;
            $second = isset($args[5]) ? $args[5] : 0;

        } else {

            if (isset($args[0])) {
                $ts = (is_int($args[0])) ? $args[0] : strtotime($args[0]);
            } else {
                $ts = time();
            }

            if (!$ts) {
                return false;
            }

            list($year, $month, $day, $hour, $minute, $second) = explode('-', date('Y-m-d-H-i-s', $ts));

        }

        return new App_Gantt_Calendar_CalendarDay($year, $month, $day, $hour, $minute, $second);

    }

    function today()
    {
        return $this->date('today');
    }

    function now()
    {
        return $this->today();
    }

    function tomorrow()
    {
        return $this->date('tomorrow');
    }

    function yesterday()
    {
        return $this->date('yesterday');
    }

}