...
class App_Gantt_Gantt
{
    ...
    function __construct($definitions)
    {
        ...
        $this->title();
        $this->grid();
        $this->groups(); // draws tasks and phases
        //$this->tasks(); // draws groups and phases
    ...
    function groups()
    {
        ...
        foreach ($this->groups['group'] as $cod => $task) {
            ...
            //$this->milestones($cod);
            //loop group task
            //echo "<br /><br />=>" . $this->groups['group'][$cod]['name'] . "($cod)<br />";
            $this->tasks($cod); // draws groups and phases
        }
        //exit;
    }
    ...
    function tasks($group)
    {
        ...
        //$this->y = $this->definitions['grid']['y'] + 40;
        $y = &$this->y;
        if (isset($this->groups['group'][$group]['task'])) {
            foreach ($this->groups['group'][$group]['task'] as $phase => $cod) {
                //foreach ($this->tasks['task'] as $cod=>$phases) {
                //foreach ($this->tasks['task'] as $cod=>$phases) {
                //echo "==>" . $this->tasks['task'][$cod]['name'] . "($group)(170)<br />";
                if ($this->definitions["not_show_tasks"] != true) {
                    $y = &$this->y;
                    $x = (($this->tasks['task'][$cod]['start'] - $this->limit['start']) / (60 * 60 * 24)) * $this->cell + $start_grid;
                    //echo "==>$y - $x ($group)(177)<br />";
                    $x2 = (($this->tasks['task'][$cod]['end'] - $this->tasks['task'][$cod]['start']) / (60 * 60 * 24)) * $this->cell + $x;
                    //echo "$x : $x2";
                    $this->rectangule($x, $y, $x2, $y + 6, $this->tasks['color'], @$this->tasks['alpha']);
                    $y2 = $y + 7;
                    $this->polygon(array($x, $y2, $x + 10, $y2, $x, $y + 15), 3, $this->tasks['color'],
                        @$this->tasks['alpha']);
                    $this->polygon(array($x2 - 10, $y2, $x2, $y2, $x2, $y + 15), 3, $this->tasks['color'],
                        @$this->tasks['alpha']);
                    $y2 = $y + $this->definitions['row']['height'] / 2;
                    ...
                    $this->rectangule(0, $y, $start_grid - 1, $y + $this->definitions['row']['height'] / 2,
                        $this->tasks['bg_color']);
                    //$this->text($this->tasks['task'][$cod]['name'],5,$y+$this->definitions['row']['height']/4-6,$this->definitions["task"]['text_color']);
                    $this->text(@$this->tasks['task'][$cod]['name'], 8,
                        $y + $this->definitions['row']['height'] / 4 - 6, @$this->definitions["task"]['text_color']);
    ...
    function phases($task)
    {
        $start_grid = $this->definitions['grid']['x'];
        $y = &$this->y;
        if (isset($this->tasks['task'][$task]['phase'])) {
            //print_r($this->progress);
            foreach ($this->tasks['task'][$task]['phase'] as $phase => $cod) {
                //echo "===>" . $this->planned['phase'][$cod]['name'] . "(213)<br />";
                // name of phase
                $this->text($this->planned['phase'][$cod]['name'], 15, $y + 15,
                    $this->definitions["phase"]['text_color']);
                ...
                $this->rectangule($t, $w1, $t2, $w2, $this->planned_adjusted['color'],
    ...
    function milestones($task)
    {
        $y = &$this->y;
        if (@is_array($this->tasks['task'][$task]['milestone'])) {
            foreach ($this->tasks['task'][$task]['milestone'] as $milestone => $cod) {
                //echo "======> milestones (276) <br />";
                ...
                $y += $this->definitions['row']['height'] / 2;
                //echo "$x : $x2";
                //$this->rectangule($x,$y,$x2,$y+6,$this->tasks['color']);
            }
        }
    }
    ...
    function grid()
    {
        ...
        foreach ($months as $month => $startdate) {
            ...
            $n_d = (int)($n_m - $this->limit['start']) / (86400);
            //echo $n_d."<br>";
            if ($n_m >= $this->limit['end']) {
                $x2 = $this->img_width - 20;
            } else {
                if ($ix1 == 0) {
                    $x2 = $n_d * $this->cell + $x1; // - 228;
                } else {
                    $x2 = $n_d * $this->cell + $x1;
                }
            }
...
//        die;
        ...
        if ($this->limit['detail'] == 'd') {
            while ($start <= $end) {
                ...
                if ($this->isHoliday($start)) {
                    $this->rectangule($x, $y + 20, $x2, $y + 40, $this->holiday_color);
                    $this->rectangule($x, $y + 41, $x2, $y2, $this->holiday_color);
                } else {
                    $this->rectangule($x, $y + 20, $x2, $y + 40, $this->grid_color);
                    $this->rectangule($x, $y + 41, $x2, $y2, $this->grid_color);
        ...
        if ($this->limit['detail'] == 'w') {
            ...
            while ($start < $end) {
                ...
                while ($startInitial < $n_w) {
                    $month2 = date("m", $startInitial);
                    $day2 = date("d", $startInitial);
                    $year2 = date("Y", $startInitial);
                    $x3 = $xx + $this->cell;
                    if ($this->isHoliday($startInitial)) {
                        $this->rectangule($xx, $y + 20, $x3, $y + 40, $this->holiday_color);
                        $this->rectangule($xx, $y + 41, $x3, $y2, $this->holiday_color);
                    }
                    if ($day2 == '19' && $month2 == '2') {
                        $startInitial = mktime(0, 0, 0, 2, 20, $year2);
                    } else {
                        $startInitial += 86400;
                    }
                    $xx = $x3;
    ...
    function definesize()
    {
        if ($this->limit['detail'] == 'm') {
            ...
            //echo date('w',$this->limit['start']);
            $this->limit['end'] = mktime(0, 0, 0, date('m', $this->limit['end']), date('t', $this->limit['end']),
                date('Y', $this->limit['end']));
    ...
    function title()
    {
        ...
        if ($this->definitions['title']['ttfont']['file']) {
            ...
            imagettftext($this->img, $font_size, 0, $this->definitions['grid']['x'] /*$this->img_width / 2 - $xdiff*/,
    ...
    function text($string, $x, $y, $color = 0)
    {
        ...
        if (@$this->definitions['text']['ttfont']['file']) {
            imagettftext($this->img, $font_size, 0, $x, $y + $font_size, $color,
                $this->definitions['text']['ttfont']['file'],
                substr($string, 0, 61) . (strlen($string) > 61 ? "..." : ""));
        } else {
            imagestring($this->img, $this->definitions['text_font'], $x, $y,
                substr($string, 0, 61) . (strlen($string) > 61 ? "..." : ""), $color);
        }
    }
            ...
            case 'jpg':
                if (function_exists("imagejpeg")) {
                    ...
                    if (isset($filename)) {
                        ...
                        imagejpeg($this->img, null, $jpg_quality);
                    }
                }
                break;
            case 'wbmp':
    ...
    {
        return isset($this->holidays) && (in_array(date('m-d', $day), $this->holidays, true)
                || in_array(date('Y-m-d', $day), $this->holidays, true));
    }
}
