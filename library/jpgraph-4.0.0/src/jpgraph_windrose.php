    ...
    function FixupIndexes($aDataArray, $num)
    {
        ...
        foreach ($aDataArray as $idx => $data) {
            if (is_string($idx)) {
                ...
                if ($res === false) {
                    JpGraphError::RaiseL(22011,
                        $idx); //('Windrose index must be numeric or direction label. You have specified index='.$idx);
                }
                $idx = $res;
                if ($idx % (16 / $num) !== 0) {
                ...
                if (in_array($idx, $keys, 1)) {
                    JpgraphError::RaiseL(22013,
                        $idx); //('You have specified the look&feel for the same compass direction twice, once with text and once with index (Index='.$idx.')');
                }
            }
            if ($idx < 0 || $idx > 15) {
    ...
    function _StrokeRegularRose($dblImg, $value, $scaling, $xc, $yc, $r, $ri)
    {
        ...
        foreach ($this->iData as $idx => $legdata) {
            ...
            if (is_string($idx)) {
                ...
                if (in_array($idx, $keys, 1)) {
                    JpgraphError::RaiseL(22018,
                        $idx);//('You have specified data for the same compass direction twice, once with text and once with index (Index='.$idx.')');
