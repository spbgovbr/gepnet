...
class Footer
{
    ...
    function Stroke($aImg)
    {
        ...
        if ($this->iTimer != null) {
            $this->right->Set($this->right->t . sprintf('%.3f',
                    $this->iTimer->Pop() / 1000.0) . $this->itimerpoststring);
        }
...
class Graph
{
        ...
        if ($aImgFormat == 'auto') {
            ...
            if (!$e) {
                JpGraphError::RaiseL(25018,
                    $aFileName);//('Incorrect file name for Graph::SetBackgroundImage() : '.$aFileName.' Must have a valid image extension (jpg,gif,png) when using autodetection of image type');
            }
            ...
            if ($aImgFormat == 'jpeg') {
                ...
                JpGraphError::RaiseL(25019,
                    $aImgFormat);//('Unknown file extension ($aImgFormat) in Graph::SetBackgroundImage() for filename: '.$aFileName);
            }
        }
    ...
    function SetY2Scale($aAxisType = 'lin', $aY2Min = 1, $aY2Max = 1)
    {
        if ($aAxisType == 'lin') {
            ...
            JpGraphError::RaiseL(25023,
                $aAxisType);//("JpGraph: Unsupported Y2 axis type: $aAxisType\nMust be one of (lin,log,int)");
        }
    ...
    function SetYScale($aN, $aAxisType = "lin", $aYMin = 1, $aYMax = 1)
    {
        ...
        if ($aAxisType == 'lin') {
            ...
            JpGraphError::RaiseL(25024,
                $aAxisType);//("JpGraph: Unsupported Y axis type: $aAxisType\nMust be one of (lin,log,int)");
        }
        ...
        if (empty($_GET[_CSIM_DISPLAY])) {
            ...
            if ($this->csimcachename != '') {
                ...
                if (file_exists($dir) && !is_writeable($dir)) {
                    JpgraphError::RaiseL(25028,
                        $dir);//('Apache/PHP does not have permission to write to the CSIM cache directory ('.$dir.'). Check permissions.');
                }
                ...
                if ($fh = @fopen($basecsim, 'w')) {
                    ...
                    JpGraphError::RaiseL(25029,
                        $basecsim);//(" Can't write CSIM \"$basecsim\" for writing. Check free space and permissions.");
                }
            } else {
    ...
    function Stroke($aStrokeFileName = '')
    {
        ...
        if ($this->iDoClipping) {
            ...
            if ($this->img->a == 0) {
                ...
                JpGraphError::RaiseL(25035,
                    $this->img->a);//('You have enabled clipping. Cliping is only supported for graphs at 0 or 90 degrees rotation. Please adjust you current angle (='.$this->img->a.' degrees) or disable clipping.');
            }
            $this->img->Destroy();
    ...
    function SetAxisLabelBackground(
        $aType,
        $aXFColor = 'lightgray',
        $aXColor = 'black',
        $aYFColor = 'lightgray',
    ...
    static function LoadBkgImage($aImgFormat = '', $aFile = '', $aImgStr = '')
    {
        ...
        if (($ext == 'jpg' && !($supported & IMG_JPG)) ||
            ...
            JpGraphError::RaiseL(25037,
                $aFile);//('The image format of your background image ('.$aFile.') is not supported in your system configuration. ');
        }
...
class Grid
{
    ...
    function Stroke()
    {
        if ($this->showMinor && !$this->scale->textscale) {
            ...
            $this->DoStroke($this->scale->ticks->maj_ticks_pos, $this->majortype, $this->majorcolor,
                $this->majorweight);
        } else {
            $this->DoStroke($this->scale->ticks->maj_ticks_pos, $this->majortype, $this->majorcolor,
                $this->majorweight);
    ...
    function DoStroke($aTicksPos, $aType, $aColor, $aWeight)
    {
        ...
        if ($this->scale->type == 'y') {
            ...
            JpGraphError::RaiseL(25054,
                $this->scale->type);//('Internal error: Unknown grid axis ['.$this->scale->type.']');
        }
        return true;
    }
} // Class
    ...
    function Stroke($aOtherAxisScale, $aStrokeLabels = true)
    {
        ...
        if ($this->scale->type == "x") {
            ...
            if ($this->title_adjust == 'high') {
                ...
                $this->title->SetPos($x,
                    ($this->img->height - $this->img->top_margin - $this->img->bottom_margin) / 2 + $this->img->top_margin,
                    $halign, "center");
            } elseif ($this->title_adjust == 'low') {
                $this->title->SetPos($x, $this->img->height - $this->img->bottom_margin, $halign, 'bottom');
            } else {
                JpGraphError::RaiseL(25061,
                    $this->title_adjust);//('Unknown alignment specified for Y-axis title. ('.$this->title_adjust.')');
            }
        }
            ...
            if (($i % $this->label_step) == 0) {
                ...
                if ($this->scale->type == "x") {
                    ...
                    if ($this->labelPos == SIDE_LEFT) { // To the left of y-axis
                        ...
                        $this->img->StrokeText($aPos - $this->tick_label_margin, $tpos, $label, $this->label_angle,
    ...
    function IntAutoScale($img, $min, $max, $maxsteps, $majend = true)
    {
        ...
        switch ($r) {
            ...
            case 3:
                ...
                break;
            default:
                JpGraphError::RaiseL(25073,
                    $r);//('Internal error. Integer scale algorithm comparison out of bound (r=$r)');
        }
    }
