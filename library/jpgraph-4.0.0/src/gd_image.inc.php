...
class Image
{
    ...
    function __construct($aWidth = 0, $aHeight = 0, $aFormat = DEFAULT_GFORMAT, $aSetAutoMargin = true)
    {
        ...
        if (!$this->SetImgFormat($aFormat)) {
            JpGraphError::RaiseL(25081,
                $aFormat);//("JpGraph: Selected graphic format is either not supported or unknown [$aFormat]");
        }
        $this->ttf = new TTF();
    ...
    function CreateRawCanvas($aWidth = 0, $aHeight = 0)
    {
        ...
        if ($aWidth <= 1 || $aHeight <= 1) {
            JpGraphError::RaiseL(25082, $aWidth,
                $aHeight);//("Illegal sizes specified for width or height when creating an image, (width=$aWidth, height=$aHeight)");
        }
    ...
    {
        $this->CopyCanvasH($this->img, $fromImg, $toX, $toY, $fromX, $fromY, $toWidth, $toHeight, $fromWidth,
            $fromHeight);
    }
...
//                JpGraphError::RaiseL(25087);//('This PHP build has not been configured with TTF support. You need to recompile your PHP installation with FreeType support.');
    ...
    function StrokeBoxedText(
        ...
        if ($shadowcolor) {
            ...
            if ($fcolor) {
                ...
                $this->FilledRoundedRectangle($x - $xmarg, $y - $ymarg, $x + $width, $y + $height - $ymarg,
                    $cornerradius);
            ...
            if (preg_match('/\n/', $txt)) {
                ...
                for ($i = 0; $i < count($tmp); ++$i) {
                    ...
                    if ($paragraph_align == "left") {
                        ...
                        imagestring($this->img, $use_font, $x + ($w - $w1), $y - $h + 1 + $i * $fh, $tmp[$i],
                            $this->current_color);
                    } else {
                        imagestring($this->img, $use_font, $x + $w / 2 - $w1 / 2, $y - $h + 1 + $i * $fh, $tmp[$i],
                            $this->current_color);
                    }
                }
            } else {
    ...
    function FilledArc($xc, $yc, $w, $h, $s, $e, $style = '')
    {
        ...
        if (abs($s - $e) > 0) {
            imagefilledarc($this->img, round($xc), round($yc), round($w), round($h), $s, $e, $this->current_color,
                $style);
    ...
    function CakeSlice($xc, $yc, $w, $h, $s, $e, $fillcolor = "", $arccolor = "")
    {
        ...
        if ($s == $e) {
            ...
            if ($arccolor != "") {
                ...
                // the filled arc.
                imagefilledarc($this->img, $xc, $yc, 2 * $w, 2 * $h, $s, $e, $this->current_color,
...
//        $this->DrawLine($this->img, $x1, $y1, $x2, $y2, $this->line_weight, $this->current_color);
    ...
    function Stream($aFile = null)
    {
        ...
        if ($this->img_format == "jpeg" && $this->quality != null) {
            ...
            if (!$res) {
                if ($aFile != null) {
                    JpGraphError::RaiseL(25107,
                        $aFile);//("Can't write to file '$aFile'. Check that the process running PHP has enough permission.");
                } else {
    ...
    function DoSupersampling()
    {
        ...
        imagecopyresampled($dst_img, $this->img, 0, 0, 0, 0, $this->original_width, $this->original_height,
    ...
    function SetImgFormat($aFormat, $aQuality = 75)
    {
        ...
        if ($aFormat == "auto") {
            ...
            if (!$tst) {
                JpGraphError::RaiseL(25110,
                    $aFormat);//(" Your PHP installation does not support the chosen graphic format: $aFormat");
            }
        }
    }
...
//        $this->DrawImageSmoothArc($im, $x2, $y2, $weight, $weight, 0, 360, $color);
    ...
    function DrawImageSmoothArc($im, $xc, $yc, $w, $h, $s, $e, $color, $style = null)
    {
        ...
        $s = (360 - $e) / 180 * M_PI;
        $e = (360 - $tmp) / 180 * M_PI;
        return imageSmoothArc($im, round($xc), round($yc), round($w), round($h),
            $this->CreateColorForImageSmoothArc($color), $s, $e);
    }
...
class ImgStreamCache
{
    ...
    function StreamImgFile($aImage, $aCacheFileName)
    {
        ...
        if ($fh = @fopen($aCacheFileName, 'rb')) {
            ...
            JpGraphError::RaiseL(25117,
                $aCacheFileName);//(" Can't open cached image \"$aCacheFileName\" for reading.");
        }
    }
    ...
    function MakeDirs($aFile)
    {
        ...
        for ($i = sizeof($dirs) - 1; $i >= 0; $i--) {
            if (!@mkdir($dirs[$i], 0777)) {
                JpGraphError::RaiseL(25118,
                    $aFile);//(" Can't create directory $aFile. Make sure PHP has write permission to this directory.");
            }
