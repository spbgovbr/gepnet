                    ...
                    for ($i = 0; $i < $n; ++$i, $x += $width, $datestamp += $minint) {
                        if ($day == 6 || $day == 0) {
                            ...
                            if ($this->iUsePlotWeekendBackground) {
                                $img->FilledRectangle($x, $yt + $this->day->iFrameWeight, $x + $width,
    ...
    function StrokeMonths($aYCoord, $getHeight = false)
    {
        if ($this->month->iShowLabels) {
            ...
            if ($this->GetMonthNbr($this->iStartDate) == $this->GetMonthNbr($this->iEndDate)
                ...
                $monthwidth = $this->GetDayWidth() * ($this->GetNumDaysInMonth($monthnbr,
                            $year) - $this->GetMonthDayNbr($this->iStartDate) + 1);
            }
    ...
    function Stroke($aImg, $aScale)
    {
        ...
        if ($this->iBreakStyle) {
            ...
            if ($this->iShadow) {
                ...
                $prect->SetPos(new Rectangle($xt + 1, $yt + 1, $xb - $xt - $this->iShadowWidth - 2,
                    $yb - $yt - $this->iShadowWidth - 2));
                $prect->Stroke($aImg);
            } else {
