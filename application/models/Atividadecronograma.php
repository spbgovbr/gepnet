<?php

/**
 * Automatically generated data model
 *
 * This class has been automatically generated at "" @ 16-05-2013 17:21
 */
class Default_Model_Atividadecronograma extends App_Model_ModelAbstract
{

    public $idatividadecronograma = null;
    public $idprojeto = null;
    public $idgrupo = null;
    public $idpredecessora = null;
    public $numseq = null;
    public $numpercentualconcluido = null;
    public $nomatividadecronograma = null;
    public $datiniciobaseline = null;
    public $datfimbaseline = null;
    public $datinicio = null;
    public $datfim = null;
    public $idresponsavel = '0';
    public $domtipoatividade = null;
    public $flacancelada = null;
    public $desobs = null;
    public $idcadastrador = null;
    public $datcadastro = null;
    public $idmarcoanterior = null;
    public $numdias = null;
    public $vlratividadebaseline = '0::bigint';
    public $vlratividade = '0::bigint';
    public $nomresponsavel = null;
    public $numfolga = '0';
    public $descriterioaceitacao = null;
    public $flaaquisicao = null;
    public $flainformatica = null;
    public $idelementodespesa = null;
    public $idpredecessora2 = null;
    public $datatividadeconcluida = null;

}

