...
//            }
    ...
    public function editarAtividadeAction() {
        $service = new Projeto_Service_AtividadeCronograma();
        $form = $service->getFormAtividade($this->_request->getParams());
        $success = false;
        if ($this->_request->isPost()) {
            $dados = $this->_request->getPost();
                ...
                foreach ($retorgaProjeto as $retorgaProjetos) {
                    ...
                    foreach ($retornaPredec as $retornaPredecs) {
                        ...
                        foreach ($retornaAtividade as $retornaAtividades) {
                            ...
                            $atualizarMultPrede = new Default_Model_DbTable_Atividadecronograma();
            ...
            //Zend_Debug::dump($dados['numfolga']);die;
            //$atividade = $service->atualizarAtividade($dados);
            //@todo inserir pessoas interessadas
            if ($update) {
    ...
    public function excluirAtividadeAction() {
        ...
        if ($this->_request->isPost()) {
            ...
            $retornaPred = $Epredecessora->retornaPredecePorIdAtividade(array('idatividadecronograma' => $idatividade, 'idprojeto' => $idProjeto));
            // Retora todas as predecessora por projeto
            $retornaTodasPred = $Epredecessora->retornaPoriDPredecessora(array('idatividadepredecessora' => $idatividade, 'idprojeto' => $idProjeto));
            ...
            if ($countPrede > 0 || $retornaTodasPred['idatividadepredecessora'] == $idatividade) {
                if ($countPrede > 0) {
                    ...
                    $msg = "ativiPredec";
                    //$this->_helper->json->sendJson($msg);
                } else if ($retornaTodasPred['idatividadepredecessora'] == $idatividade) {
            ...
            } else if ($retornaTodasPred['idatividadepredecessora'] == '') {
                $success = true;
                $msg = 'atividade';
