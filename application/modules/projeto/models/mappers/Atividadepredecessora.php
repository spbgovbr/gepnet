...
class Projeto_Model_Mapper_Atividadepredecessora extends App_Model_Mapper_MapperAbstract
{
    ...
    public function excluirPorProjeto($params)
    {
//        try {
//            $pks = array(
//                "idatividadepredecessora" => $params['idatividadepredecessora'],
...
//            );
           // $where        = $this->_generateRestrictionsFromPrimaryKeys($pks);
            $where = $this->_db->quoteInto('idatividadepredecessora = ?', $params['idatividadepredecessora'], 'and idprojeto = ?', $params['idprojeto']);
            $retorno      = $this->getDbTable()->delete($where);
            return $retorno;
//        } catch ( Exception $exc ) {
//            throw $exc;
//        }
        ...
        $sql = "SELECT 
		    idatividadepredecessora
                ...
                WHERE
                    ...
                    and p.idatividade in ($idatividade)
                   ";
        //Zend_Debug::dump($sql); die;
        $resultado = $this->_db->fetchAll($sql);
        return $resultado;
  ...
  //////////////////////////// MULTIPLAS PREDECESSORAS//////////////////////////  
    ...
    {
        $sql = "select 
                    max(to_char(cron.datfim, 'DD/MM/YYYY')) as datfim
                from 
                    agepnet200.tb_atividadecronograma cron
                where cron.idatividadecronograma = :idatividadecronograma
                and cron.idprojeto = :idprojeto
                group by cron.idatividadecronograma";
