...
//            );
            //$where   = $this->_generateRestrictionsFromPrimaryKeys($pks);
         $where = $this->_db->quoteInto('idatividadecronograma = ?', $params['idatividadecronograma'], 'and idprojeto = ?', $params['idprojeto']);
            $retorno = $this->getDbTable()->delete($where);
            return $retorno;
//        } catch ( Exception $exc ) {
//            throw $exc;
...
//        $tipoAtividade = explode(',', $params['domtipoatividade']);
 ...
 // Trazendo todas as entregas por grupo
    public function retornaIdEntregaPorGrupo($params){
        $sql = " select idatividadecronograma
                   from agepnet200.tb_atividadecronograma
                        where idprojeto = :idprojeto 
                        and idgrupo = :idgrupo
                        and domtipoatividade = 2";
