...
class Planejamento_Service_Portfolio extends App_Service_ServiceAbstract
{
     ...
     */
    public function getFormPortfolioEstrategico($params)
    {
    ...
    public function getFormPesquisarPortfolio()
    {
        ...
        $form->noportfolio->setRequired(false)->setAttribs(array('data-rule-required' => false,
                                                                 'class' => 'span2_5'));
    ...
    {
        $form = $this->_getForm('Planejamento_Form_PortfolioEstrategico');
        $form->idobjetivo->setValue($params['idobjetivo']);
        $form->idescritorio->setValue($params['idescritorio']);
   ...
   /* public function editarPortfolioPrograma($dados)
    ...
    {    
         ...
         foreach($objetivos as $i => $objetivo) {
             ...
             $objetivosArray[$i]['totalProjetoProposta'] = $mapperGerencia->getQtdeProjetosPorStatus(array('idobjetivo' => $objetivo->idobjetivo,  'domstatusprojeto' => Projeto_Model_Gerencia::STATUS_PROPOSTA,
                                                                                                           $parametro => $parametros));
             $objetivosArray[$i]['totalProjetos'] = $mapperGerencia->getTotalProjetosPorObjetivo($objetivo->idobjetivo, array($parametro => $parametros));
             foreach($objetivo->acoes as $j => $acao) {
                 $contAcao++;
                 $ac[$j] = $acao->toArray();
                 $ac[$j]['totalProjetoProposta'] = $mapperGerencia->getQtdeProjetosPorStatus(array('idacao' => $acao->idacao,  'domstatusprojeto' => Projeto_Model_Gerencia::STATUS_PROPOSTA,
                                                                                                   $parametro => $parametros));
                 $ac[$j]['totalProjetos'] = $mapperGerencia->getQtdeProjetosPorStatusAcao(array('idacao' => $acao->idacao,  
    ...
    {    
         ...
         foreach($objetivos as $i => $objetivo) {
             ...
             $objetivosArray[$i] = $objetivo->toArray();
             $objetivosArray[$i]['totalProjetoProposta'] = $mapperGerencia->getPesquisaQtdeProjetosPorStatus(array($parametro => $parametros,'domstatusprojeto' => Projeto_Model_Gerencia::STATUS_PROPOSTA));
             $objetivosArray[$i]['totalProjetos'] = $mapperGerencia->getTotalProjetosPorObjetivo($objetivo->idobjetivo, array($parametro => $parametros));
             foreach($objetivo->acoes as $j => $acao) {
                 $contAcao++;
                 $ac[$j] = $acao->toArray();
                 $ac[$j]['totalProjetoProposta'] = $mapperGerencia->getQtdeProjetosPorStatus(array($parametro => $parametros,'domstatusprojeto' => Projeto_Model_Gerencia::STATUS_PROPOSTA));
                 $ac[$j]['totalProjetos'] = $mapperGerencia->getQtdeProjetosPorStatusAcao(array($parametro => $parametros,'domstatusprojeto' =>  Projeto_Model_Gerencia::STATUS_ANDAMENTO .',' .
                                                                                                                    Projeto_Model_Gerencia::STATUS_CONCLUIDO . ',' .      
                                                                                                                    Projeto_Model_Gerencia::STATUS_PARALISADO . ',' .      
                                                                                                                    Projeto_Model_Gerencia::STATUS_CANCELADO . ',' .      
                                                                                                                    Projeto_Model_Gerencia::STATUS_PROPOSTA,      
